...
<div class="estimate-button-container" style="margin-left:13px; display:flex; gap:8px; margin-top:-2px;">
  ...
  <button class="delete" on:click={onDelete}
    ...
    style="height:30px; width:160px; border-radius:6px; color:white; border:1px solid #6c757d; box-sizing:border-box; background-color:#d9534f; opacity:{(!hasSel || isBlocked)?0.5:1}; cursor:{(!hasSel || isBlocked)?'not-allowed':'pointer'};">
    Delete
  </button>
  ...
  <button class="send" on:click={onSend}
    ...
    style="height:30px; width:160px; border-radius:6px; color:white; border:1px solid #6c757d; box-sizing:border-box; background-color:#0d6efd; opacity:{canSendCommitBtn?1:0.5}; cursor:{canSendCommitBtn?'pointer':'not-allowed'};">
    Send &amp; Commit
  </button>
  ...
  <button class="handover" on:click={onHandover}
    ...
    style="height:30px; width:170px; border-radius:6px; color:white; border:1px solid #6c757d; box-sizing:border-box; background-color:#fd7e14; opacity:{canHandover?1:0.5}; cursor:{canHandover?'pointer':'not-allowed'};">
    Handover to Booking
  </button>
</div>
